...
Public Class PianoInclinatoTallone
...
#Region " Constructor "
    ...
    Public Sub New(position As ePosizionePianiInclinato, spessore As Double)
        MyBase.New(eTipoPianoInclinato.tallone, position, spessore)
    End Sub
...
#Region " Passi procedura "
    ...
    Public Sub SetBordoInterno(side As eSide)
        ...
        If getObjects.ObjectCount <> 1 Then Throw New Exception("Impossibile creare la curca P1-C")
        Dim curveObjRefP1_C = New MRhinoObjRef(getObjects.Object(0))
'#If DEBUG
'        AddDocumentToDebug(curveObjRefP1_C.Curve().DuplicateCurve(), "curveObjRefP1_C")
'#End If
        App.RunScript("_SelNone", 0)
        ...
        getObjects.SetGeometryFilter(IRhinoGetObject.GEOMETRY_TYPE_FILTER.curve_object)
        getObjects.GetObjects(0, Integer.MaxValue)
        If getObjects.ObjectCount <> 1 Then Throw New Exception("Impossibile creare la curva C-E1")
        Dim curveObjRefC_E1 = New MRhinoObjRef(getObjects.Object(0))
'#If DEBUG
'        AddDocumentToDebug(curveObjRefC_E1.Curve().DuplicateCurve(), "curveObjC_E1")
'#End If
        App.RunScript("_SelNone", 0)
        curveObjRefP1_C.Object().Select(True, True)
        curveObjRefC_E1.Object().Select(True, True)
        ...
        App.RunScript("_SelNone", 0)
        getObjects.Dispose()
        curveObjRefP1_C.Dispose()
        curveObjRefC_E1.Dispose()
    ...
    Public Sub Set_A_B_C_D_E()
        ...
        'Dim L2 = pointCT.DistanceTo(pointCV)       
        Dim t as Double
        ...
        Dim vectorL2 = New On3dPoint(pointCV) - New On3dPoint(pointCT)
        nearPoint = New On3dPoint(pointCT.x + vectorL2.x * 6 / 10, pointCT.y + vectorL2.y * 6 / 10, pointCT.z + vectorL2.z * 6 / 10)
        insoleBottomAxis.GetClosestPoint(nearPoint, t)
        Dim pointC = insoleBottomAxis.PointAt(t)
        IdPointC = Doc.AddPointObject(pointC).Attributes.m_uuid

        nearPoint = New On3dPoint(pointC.x + vectorL2.x * 15 / 100, pointC.y + vectorL2.y * 15 / 100, pointC.z + vectorL2.z * 15 / 100)
        insoleBottomAxis.GetClosestPoint(nearPoint, t)
        Dim pointD = insoleBottomAxis.PointAt(t)
        IdPointD = Doc.AddPointObject(pointD).Attributes.m_uuid

        Dim vectorCV_P2 = New On3dPoint(pointP2) - New On3dPoint(pointCV)
        vectorCV_P2.Unitize()
        nearPoint = New On3dPoint(vectorCV_P2.x * 10 + pointCV.x, vectorCV_P2.y * 10 + pointCV.y, vectorCV_P2.z * 10 + pointCV.z)
        insoleBottomAxis.GetClosestPoint(nearPoint, t)
    ...
    ''' <summary>
    ''' SELEZIONE PUNTI CT E CV CON IMPOSTAZIONE SNAP - VEDI procedura 4
    ''' </summary>
    Public Sub Set_CT_CV()
        ...
        IdLanguageManager.PromptUserMessage("Selezionare i punti CT(centro tallone) e CV(centro del volta) e tasto Enter")
        'COMANDO
        ...
        IdPointCV = pointList.OrderBy(Function(keyValue) keyValue.Value.x).Select(Function(keyValue) keyValue.Key).Last()
...
#Region " IClonable "
    ...
    Public Overrides Function Clone() As Object
        Dim res As New PianoInclinatoTallone(Me.Position, Me.Spessore)
